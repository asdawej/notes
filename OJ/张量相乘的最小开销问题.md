...
考虑如下的一张 dp 表, 我们已经填入了初值; 表中最右上角的红色即为我们的目标解:

$$\begin{bmatrix}
0 & \infty & \infty & \cdots & \textcolor{red}{\infty} \\
& 0 & \infty & \ddots & \vdots \\
& & 0 & \ddots & \vdots \\
& & & \ddots & \infty \\
& & & & 0
...
根据状态方程, 对于一个待求的 `dp(i, j)` (表中黄色), 我们需要知道在第 `i` 行和第 `j` 列上介于 `dp(i, j)` 和 `dp(i, i)`、`dp(j, j)` 的值 (表中绿色):

$$\begin{bmatrix}
\ddots \\
& 0 & \textcolor{green}{\cdots} & \textcolor{yellow}{\infty} \\
& & \ddots & \textcolor{green}{\vdots} \\
& & & 0 \\
& & & & \ddots
\end{bmatrix}$$
